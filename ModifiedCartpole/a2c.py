...
class A2C(nn.Module):
    ...
    def __init__(self, env, hidden_size=64, gamma=.99, random_seed=None, Riemannian=False, m=16,n=16,r=16):
        """
        ...
        ('linear_in', nn.Linear(self.in_size, m)),
        ('relu1', nn.ReLU()),
        ('linear_1_rie', nn.Linear(m, r, bias = False)),
        ('linear_1', nn.Linear(r, n)),
        ('relu3', nn.ReLU()),
        ('linear_out', nn.Linear(n, 1))
        ])).double()
    ...
    def train_env_episode(self, render=False):
        ...
        # Standardize rewards
        rewards = f(rewards)
        rewards = (rewards - torch.mean(rewards)) / (torch.std(rewards) + .000000000001) # it is important
